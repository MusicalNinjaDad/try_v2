...
fn impl_derive(input: TokenStream2) -> DiagnosticStream {
    ...
    let (name, _, output_variant_name, output_type, _, residual_type) = tryenum.split_for_impl();
...
fn impl_convert_result(input: TokenStream2) -> DiagnosticStream {
    ...
    let tryenum = TryEnum::parse(&ast)?;
    let (name, _, _, _, output_type_name, residual_type) = tryenum.split_for_impl();
...
fn impl_iterator_traits(input: TokenStream2) -> DiagnosticStream {
    ...
    let tryenum = TryEnum::parse(&ast)?;
    let (name, _, output_variant_name, output_type, output_type_name, _) = tryenum.split_for_impl();
